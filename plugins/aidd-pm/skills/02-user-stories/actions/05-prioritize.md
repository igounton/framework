...
The stories as a single ordered list, each with a priority rank, ordered by the method in `@../references/rating.md`.

## Process

1. **Score.** For each story, weigh delivered value against its story points.
2. **Order.** Sort by descending value-to-effort, then break ties with the lower impact first, per `@../references/rating.md`.
