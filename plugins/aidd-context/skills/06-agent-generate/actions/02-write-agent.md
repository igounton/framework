...
1. **Build.** Copy `@../assets/agent-template.md` into one canonical agent. Strip the scaffold (comments + `<...>`).
2. **Frontmatter.** Apply the per-tool frontmatter from `@../references/tool-paths.md`. Drop a field where the tool does not support it. Plugin source: keep the canonical frontmatter.
3. **Convert.** Apply any per-tool structural conversion listed in `@../references/tool-paths.md`. Plugin source skips conversion.
