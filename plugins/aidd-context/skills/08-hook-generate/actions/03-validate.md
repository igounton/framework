...
2. **Moment.** Confirm the event name is the right one for that tool's moment, and the matcher is well-formed (`@../references/tool-paths.md`).
3. **Blocking.** If the hook blocks, confirm the moment can block on that tool (`@../references/tool-paths.md`).
