...
1. **Enumerate rules.** Run the `list-rules.mjs` script located at `scripts/list-rules.mjs` under the `11-discovery` skill root (resolve via the plugin install path) from the user's project root; accept a silent empty array when no surface contains rules.
