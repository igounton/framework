...
1. **Build.** Copy `@../assets/command-template.md` into one canonical command: single objective, under ten steps, no Role section. Strip the scaffold (comments + `<...>`).
2. **Frontmatter.** Set the fields per tool from `@../references/tool-paths.md`. Drop a field a tool does not support.
...
4. **Arguments.** If the command takes input, reference it with `$ARGUMENTS`. For a CLI call, use the injection syntax only where the target tool supports it, otherwise describe the command in the body (`@../references/command-authoring.md`).
