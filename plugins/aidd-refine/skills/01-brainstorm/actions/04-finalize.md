...
The approved refined idea with its flagged open assumptions and risks, a pointer to the fitting next step, and, when the user picks the file, a markdown file at `aidd_docs/brainstorm/<yyyy_mm_dd>-<slug>.md`.
...
5. **Offer to persist.** Once approved, present all three destinations and act on the pick. Name all three, persist nothing without the user's choice.
   - **File.** Write the document to exactly `aidd_docs/brainstorm/<yyyy_mm_dd>-<slug>.md`, where `<yyyy_mm_dd>` is today's date and `<slug>` is the idea in kebab-case. The directory and format are fixed, never another path or name. Create `aidd_docs/brainstorm/` if missing.

     | Idea, saved on | File written |
     | --- | --- |
     | aidd-craft plugin, 2026-03-09 | `aidd_docs/brainstorm/2026_03_09-aidd-craft-plugin.md` |
     | dark mode toggle, 2026-11-20 | `aidd_docs/brainstorm/2026_11_20-dark-mode-toggle.md` |
...
- When the user picks the file, a file exists afterward at `aidd_docs/brainstorm/<yyyy_mm_dd>-<slug>.md` and nowhere else.
